...
namespace CDRService
{
    ...
    public class FtpConnection
    {
        ...
        public bool UseBinaryMode
        {
            get
            {
                return mUseBinaryMode;
            }
            set
            {
                mUseBinaryMode = value;
            }
        }
        ...
        public void Upload(FileStream _fileStream, string remoteFileName)
        {
        ...
        public void Upload(FileInfo fileInfo)
        {
            //Mark the beginning of the transfer
            DateTime startTime = DateTime.Now;
            string remoteFileName = Path.GetFileName(fileInfo.Name);
            ...
            try
            {
                ...
                while (true)
                {
                    bytesRead = fileInfo.Open(FileMode.Open).Read(buffer, 0, buffer.Length);
                    if (bytesRead == 0)
                        break;
                    uploadStream.Write(buffer, 0, bytesRead);
                    bytesTotalWritten += bytesRead;
        ...
        public bool FtpDirectoryExists(string directory, string username, string password)
        {

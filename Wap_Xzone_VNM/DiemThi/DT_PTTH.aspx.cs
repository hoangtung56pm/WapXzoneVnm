...
{
    public partial class DT_PTTH : BasePage
    {
        private int width;        
        private string lang;                
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (lang == "1")
            {
                ltrDichvu.Text = "TRA ĐIỂM THI TỐT NGHIỆP PTTH 2011";
            }
            else
            {
                ltrDichvu.Text = "TRA DIEM THI TOT NGHIEP PTTH 2011";
            }
        ...
        protected void HienThiNoiDung(Boolean cuphapDung, Boolean thanhToan)
        {
            ...
            if (cuphapDung)
            {
                ...
                {                    
                    if (lang == "1")
                    {
                        ltrNoiDung.Text = noiDung.Replace("De xem thu tu xep hang, soan: VTR", "").Replace(" gui 8579. DTHT:19001255", "").Replace("<mã đối tác>", "").Replace("<mã tỉnh>", "").Replace("<SBD>", "").Replace("<mã trường>", "").Replace("_", "");
                    }
                    else
                    {
                        ltrNoiDung.Text = noiDung.Replace("De xem thu tu xep hang, soan: VTR", "").Replace(" gui 8579. DTHT:19001255", "").Replace("<mã đối tác>", "").Replace("<mã tỉnh>", "").Replace("<SBD>", "").Replace("<mã trường>", "").Replace("_", "");
            ...
            logger.Debug("Diem thi: DT_PTTH - parameter: " + price + " - " + txtMaTinh.Text.Trim() + " " + txtSoBaoDanh.Text.Trim());            
        ...
        protected void btnThucHien_Click(object sender, ImageClickEventArgs e)
        {
            if (telCo == "Undefined")
            {
                ...
                if (lang == "1")
                {
                    ltrSMS.Text = "Soạn tin <b>DTN " + (txtMaTinh.Text.Trim().Length > 0 ? txtMaTinh.Text.Trim().ToUpper() : "&lt;Mã tỉnh&gt;") + " " + (txtSoBaoDanh.Text.Trim().Length > 0 ? txtSoBaoDanh.Text.Trim() : "&lt;Số báo danh&gt;") + "</b> gửi <b>" + ConfigurationSettings.AppSettings.Get("diemthicommandcode") + "</b> để tra điểm tốt nghiệp THCS" + Resources.Resource.wChon3G;
                }
                else
                {
                    ltrSMS.Text = "Soan tin <b>DTN &lt;" + (txtMaTinh.Text.Trim().Length > 0 ? txtMaTinh.Text.Trim().ToUpper() : "&lt;Ma tinh&gt;") + " " + (txtSoBaoDanh.Text.Trim().Length > 0 ? txtSoBaoDanh.Text.Trim() : "&lt;So bao danh&gt;") + "</b> gui <b>" + ConfigurationSettings.AppSettings.Get("diemthicommandcode") + "</b> de tra diem tot nghiep THCS" + Resources.Resource.wChon3G_KD;
                }
            }
            else
            {
                DT_THPTService.ServiceController objMbox = new DT_THPTService.ServiceController();
                DT_THPTService.ResultData objResulnInfo = new DT_THPTService.ResultData();                
                objResulnInfo = objMbox.QueryDataTNPT(Session["msisdn"].ToString(), txtMaTinh.Text.Trim().ToUpper(), txtSoBaoDanh.Text.Trim(), "1", "DTN", "8579", "1");
                
                chitietGiaodich = "DT PTTH: ";

...
{
    public partial class DiemChuan : BasePage
    {
        private int width;        
        private string lang;                
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (lang == "1")
            {
                ltrDichvu.Text = "TRA ĐIỂM CHUẨN";
            }
            else
            {
                ltrDichvu.Text = "TRA DIEM CHUAN";
            }

            price = ConfigurationSettings.AppSettings.Get("diemthi10price");            
        ...
        protected void HienThiNoiDung(Boolean cuphapDung, Boolean thanhToan)
        {
            ...
            if (cuphapDung)
            {
                ...
                {                    
                    if (lang == "1")
                    {
                        ltrNoiDung.Text = noiDung.Replace("De tra diem thi, soan: DDH", "").Replace(" gui 8579.DTHT:19001255", "").Replace("<mã đối tác>", "").Replace("<SBD>", "").Replace("<mã trường>", "").Replace("_", "");
                    }
                    else
                    {
                        ltrNoiDung.Text = noiDung.Replace("De tra diem thi, soan: DDH", "").Replace(" gui 8579.DTHT:19001255", "").Replace("<mã đối tác>", "").Replace("<SBD>", "").Replace("<mã trường>", "").Replace("_", "");
            ...
            logger.Debug("Diem thi: DiemChuan - parameter: " + price + " - " + txtMaTruong.Text.Trim() + " " + txtMaNganh.Text.Trim());            
        ...
        {            
            if (telCo == "Undefined")
            {
                ...
                if (lang == "1")
                {
                    ltrSMS.Text = "Soạn tin <b>DCH " + (txtMaTruong.Text.Trim().Length > 0 ? txtMaTruong.Text.Trim().ToUpper() : "&lt;Mã trường&gt;") + " " + (txtMaNganh.Text.Trim().Length > 0 ? txtMaNganh.Text.Trim() : "&lt;Mã ngành&gt;") + "</b> gửi <b>" + ConfigurationSettings.AppSettings.Get("diemthi10commandcode") + "</b> để tra điểm chuẩn đại học" + Resources.Resource.wChon3G;
                }
                else
                {
                    ltrSMS.Text = "Soan tin <b>DCH " + (txtMaTruong.Text.Trim().Length > 0 ? txtMaTruong.Text.Trim().ToUpper() : "&lt;Ma truong&gt;") + " " + (txtMaNganh.Text.Trim().Length > 0 ? txtMaNganh.Text.Trim() : "&lt;Ma nganh&gt;") + "</b> gui <b>" + ConfigurationSettings.AppSettings.Get("diemthi10commandcode") + "</b> de tra diem chuan dai hoc" + Resources.Resource.wChon3G_KD;
                }
            }
            else
            {
                ...
                objResulnInfo = objMbox.QueryDepartmentPoint(Session["msisdn"].ToString(), txtMaTruong.Text.Trim().ToUpper(), txtMaNganh.Text.Trim().ToUpper(), "1", "DCH", "8679", "1");

                chitietGiaodich = "DiemChuan: ";
                ...
                {
                    //Sai so bao danh
                    chitietGiaodich += string.Format("Sai cu phap (truong hop sai so bao danh) voi Message la  {0}", txtMaTruong.Text.Trim() + " " + txtMaNganh.Text.Trim());
                ...
                {
                    //Sai mã tỉnh
                    chitietGiaodich += string.Format("Sai cu phap (truong hop sai ma tinh) voi Message la  {0}", txtMaTruong.Text.Trim() + " " + txtMaNganh.Text.Trim());

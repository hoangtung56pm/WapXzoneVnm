...
{
    public partial class TiLeChoi : BasePage
    {
        private int width;        
        private string lang;                
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (lang == "1")
            {
                ltrDichvu.Text = "TRA TỈ LỆ CHỌI";
            }
            else
            {
                ltrDichvu.Text = "TRA TI LE CHOI";
            }
        ...
        protected void HienThiNoiDung(Boolean cuphapDung, Boolean thanhToan)
        {
            ...
            lblTen.Text = txtMaTruong.Text.Trim();
            if (cuphapDung)
            {
                if (thanhToan)
                {                    
            ...
            logger.Debug("Diem thi: TiLeChoi - parameter: " + price + " - " + txtMaTruong.Text.Trim());            
        ...
        {            
            if (telCo == "Undefined")
            {
                ...
                if (lang == "1")
                {
                    ltrSMS.Text = "Soạn tin <b>TLC " + (txtMaTruong.Text.Trim().Length > 0 ? txtMaTruong.Text.Trim().ToUpper() : "&lt;Mã trường&gt;") +  "</b> gửi <b>" + ConfigurationSettings.AppSettings.Get("diemthicommandcode") + "</b> để tra tỉ lệ chọi đại học" + Resources.Resource.wChon3G;
                }
                else
                {
                    ltrSMS.Text = "Soan tin <b>TLC " + (txtMaTruong.Text.Trim().Length > 0 ? txtMaTruong.Text.Trim().ToUpper() : "&lt;Ma truong&gt;") + "</b> gui <b>" + ConfigurationSettings.AppSettings.Get("diemthicommandcode") + "</b> de tra ti le choi dai hoc" + Resources.Resource.wChon3G_KD;
                }
            }
            else
            {
                ...
                objResulnInfo = objMbox.QueryUniversityRate(Session["msisdn"].ToString(), txtMaTruong.Text.Trim().ToUpper(), "1", "TLC", "8579", "1");

                chitietGiaodich = "TiLeChoi: ";

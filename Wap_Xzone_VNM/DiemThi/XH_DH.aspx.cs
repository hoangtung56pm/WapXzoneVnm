...
{
    public partial class XH_DH : BasePage
    {
        private int width;        
        private string lang;                
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (lang == "1")
            {
                ltrDichvu.Text = "TRA XẾP HẠNG TRONG TRƯỜNG";
            }
            else
            {
                ltrDichvu.Text = "TRA XEP HANG TRONG TRUONG";
            }
        ...
        protected void HienThiNoiDung(Boolean cuphapDung, Boolean thanhToan)
        {
            ...
            if (cuphapDung)
            {
                ...
                {                    
                    if (lang == "1")
                    {
                        ltrNoiDung.Text = noiDung.Replace("De tra TT ho tro du doan DAU-TRUOT, soan : DHT", "").Replace(" gui 8579.DTHT:19001255", "").Replace("<mã đối tác>", "").Replace("<mã ngành>", "").Replace("<SBD>", "").Replace("<mã trường>", "").Replace("_", "");
                    }
                    else
                    {
                        ltrNoiDung.Text = noiDung.Replace("De tra TT ho tro du doan DAU-TRUOT, soan : DHT", "").Replace(" gui 8579.DTHT:19001255", "").Replace("<mã đối tác>", "").Replace("<mã ngành>", "").Replace("<SBD>", "").Replace("<mã trường>", "").Replace("_", "");
            ...
            logger.Debug("Diem thi: XH_DH - parameter: " + price + " - " + " " + txtSoBaoDanh.Text.Trim());            
        ...
        {            
            if (telCo == "Undefined")
            {
                ...
                if (lang == "1")
                {
                    ltrSMS.Text = "Soạn tin <b>XHA " + (txtSoBaoDanh.Text.Trim().Length > 0 ? txtSoBaoDanh.Text.Trim() : "&lt;Số báo danh&gt;") + "</b> gửi <b>" + ConfigurationSettings.AppSettings.Get("diemthicommandcode") + "</b> để tra vị trí xếp hạng trong trường" + Resources.Resource.wChon3G;
                }
                else
                {
                    ltrSMS.Text = "Soan tin <b>XHA " + (txtSoBaoDanh.Text.Trim().Length > 0 ? txtSoBaoDanh.Text.Trim() : "&lt;So bao danh&gt;") + "</b> gui <b>" + ConfigurationSettings.AppSettings.Get("diemthicommandcode") + "</b> de tra vi tri xep hang trong truong" + Resources.Resource.wChon3G_KD;
                }
            }
            else
            {
                ...
                objResulnInfo = objMbox.QueryOrder(Session["msisdn"].ToString(), txtSoBaoDanh.Text.Trim().ToUpper(), "1", "XHA", "8579", "1");
                
                chitietGiaodich = "XH DH: ";

    ...
    public partial class Download : BasePage
    {
        ...
        protected void HienThiNoiDung(Boolean thuchien,Boolean isLog)
        {
            ...
            {            
                ...
                lnkDownload.NavigateUrl = UrlProcess.GetDownloadItem(AppEnv.CheckFreeContentTelco(), "1", id.ToString(), SecurityMethod.MD5Encrypt(id.ToString()));
                if (ConvertUtility.ToInt32(dtDetail.Rows[0]["W_CategoryID"]) == ConvertUtility.ToInt32(ConfigurationSettings.AppSettings.Get("thuphapid")))
                {
                    ...
                    chitietGiaodich = "Thu phap: " + dtDetail.Rows[0]["WCode"].ToString() + " -- id:" + id.ToString();
                    if(isLog)
                    {

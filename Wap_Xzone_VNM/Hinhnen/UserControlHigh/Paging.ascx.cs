    ...
    public partial class Paging : System.Web.UI.UserControl
    {
        ...
        protected override void OnPreRender(EventArgs e)
        {
            ...
            if (totalrecord > 0 && pagesize > 0 && numberpage > 0)
            {
                ...
                if (1 < curpage)
                {
                    lnkFirst.NavigateUrl = UrlProcess.HinhNenChuyenMuc(ConvertUtility.ToInt32(arrUrl[3]), 1, arrUrl[5].Replace(".aspx", ""));
                    lnkPrev.NavigateUrl = UrlProcess.HinhNenChuyenMuc(ConvertUtility.ToInt32(arrUrl[3]), (curpage - 1), arrUrl[5].Replace(".aspx", ""));
                }
                if (curpage < totalpage)
                {
                    lnkLast.NavigateUrl = UrlProcess.HinhNenChuyenMuc(ConvertUtility.ToInt32(arrUrl[3]), totalpage, arrUrl[5].Replace(".aspx", ""));
                    lnkNext.NavigateUrl = UrlProcess.HinhNenChuyenMuc(ConvertUtility.ToInt32(arrUrl[3]), curpage + 1, arrUrl[5].Replace(".aspx", ""));
                }
            }
            else
            {

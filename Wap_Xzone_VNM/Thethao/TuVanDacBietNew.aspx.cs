...
{
    public partial class TuVanDacBietNew : BasePage
    {
        private int width;
        private string lang;
        ...
        protected void HienThiNoiDung(Boolean thuchien, Boolean log)
        {
            ...
            if (thuchien)
            {
                if (ds87detail.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (log) Transaction.Success(Session["telco"].ToString(), Session["msisdn"].ToString(), price, Request.Url.ToString(), gameid, chitietGiaodich, 14);//game 87
                }
            }
            else
            {

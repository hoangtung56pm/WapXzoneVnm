    ...
    public partial class Paging : System.Web.UI.UserControl
    {
        ...
        protected override void OnPreRender(EventArgs e)
        {
            ...
            if (totalrecord > 0 && pagesize > 0 && numberpage > 0)
            {
                ...
                if (1 < curpage)
                {
                    lnkFirst.NavigateUrl = Request.AppRelativeCurrentExecutionFilePath.Replace("~/UserControl/Thethao/", "") + queryparam + (1).ToString();
                    lnkPrev.NavigateUrl = Request.AppRelativeCurrentExecutionFilePath.Replace("~/UserControl/Thethao/", "") + queryparam + (curpage - 1).ToString();
                }
                if (curpage < totalpage)
                {
                    lnkLast.NavigateUrl = Request.AppRelativeCurrentExecutionFilePath.Replace("~/UserControl/Thethao/", "") + queryparam + totalpage.ToString();
                    lnkNext.NavigateUrl = Request.AppRelativeCurrentExecutionFilePath.Replace("~/UserControl/Thethao/", "") + queryparam + (curpage + 1).ToString();
                }
            }
            else
            {
        ...
        public void rptPage_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            string url = Request.AppRelativeCurrentExecutionFilePath.Replace("~/Thethao/", "") + queryparam + e.Item.DataItem;

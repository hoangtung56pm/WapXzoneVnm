...
namespace WapXzone_VNM.Video
{
    public partial class Download : BasePage
    {
        ...
        protected void HienThiNoiDung(Boolean thuchien,Boolean isLog)
        {
            ...
            chitietGiaodich = "Video: " + dtDetail.Rows[0]["VTitle_Unicode"].ToString() + " -- id:" + id.ToString();
            if (thuchien)
            {
                if (lang == "1")
                {
                ...
                lnkDownload.NavigateUrl = UrlProcess.GetDownloadItem(AppEnv.CheckFreeContentTelco(), "5", id.ToString(), SecurityMethod.MD5Encrypt(id.ToString()));

    ...
    public partial class DangKy : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                {
                    string value = AppEnv.GetRegisterService(Session["msisdn"].ToString(), "DK KQ " + madichvu);
                    if (value == "1")
                    {
                        pnlSMS.Visible = true;
                        if (lang == "1")
                        {
                            ltrHuongdan.Text = "Bóng Đá";
                            ltrSMS.Text =
                                "Bạn đã là thuê bao của dịch vụ " + GetName(madichvu) + ". Xin cảm ơn <br/> Để hủy dịch vụ vui lòng SOẠN TIN : <b> HUY KQ " + madichvu + " </b> gửi <b>" + AppEnv.GetSetting("S2ShortCode") + "</b>";
                        }
                        else
                        {
                            ltrHuongdan.Text = "Bong Da";
                            ltrSMS.Text =
                                "Ban da la thue bao cua dich vu " + UnicodeUtility.UnicodeToKoDau(GetName(madichvu)) + ". Xin cam on <br/> De huy dich vu vui long SOAN TIN : <b> HUY KQ " + madichvu + " </b> gui <b>" + AppEnv.GetSetting("S2ShortCode") + "</b>";
                        }
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if (lang == "1")
                    {
                        ...
                        ltrSMS.Text = "Hệ thống không xác định được số điện thoại của bạn. Vui lòng truy cập bằng 3G/GPRS hoặc soạn tin: DK KQ " + madichvu + " gửi " + AppEnv.GetSetting("S2ShortCode") + " (miễn phí đăng ký)";
                    }
                    else
                    {
                        ltrHuongdan.Text = "Thong Bao";
                        ltrSMS.Text = "He thong khong xac dinh duoc so dien thoai cua ban. Vui long truy cap bang 3G/GPRS hoac soan tin DK KQ " + madichvu + " gui " + AppEnv.GetSetting("S2ShortCode") + " (mien phi dang ky)";
                    }
                }
            }
        }
        ...
        private static string GetName(string input)
        {
            ...
            if (input == "HN")
                str = "Kết quả bóng đá ngày";
            else if (input == "ANH")
                str = "Giải ngoại hạng Anh";
            else if (input == "PHAP")
                str = "League 1";
            else if (input == "Y")
                str = "Serie A";

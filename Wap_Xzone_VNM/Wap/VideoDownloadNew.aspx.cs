...
{
    public partial class VideoDownloadNew : BasePage
    {
        private int width;
        ...
        void rptItem_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            imgAvatar.ImageUrl = AppEnv.GetSetting("urldata") + row["Avatar"].ToString().Replace("~/", "");
            lnkTen.NavigateUrl = lnkAvatar.NavigateUrl = AppEnv.GetSetting("urldata") + row["Path"].ToString().Replace("~/", "");
            if (lang == "1")
            {

...
{
    public partial class WallpaperDownload : BasePage
    {
        private int width;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //        if (lang == "1")
                //        {
                //            lnkfile.Text = row["Wallpaper_Name"].ToString();
                //        }
                //        else
                //        {
                //            lnkfile.Text = UnicodeUtility.UnicodeToKoDau(row["Wallpaper_Name"].ToString());
                //        }
                //        string download = AppEnv.GetSetting("VNMdownload");
                //        img.ImageUrl = "http://media.xzone.vn/" + row["Path"].ToString().Replace("~/", "");// "" + ;
                //        //img.Width = 60;
                //        img.Attributes.Add("style", "width:60px;padding-left:15px;");
                //        lnkfile.NavigateUrl = download + "?type=1&id=" + row["ID"].ToString() + "&code=" + SecurityMethod.MD5Encrypt(row["ID"].ToString()); //"http://media.xzone.vn/" + row["Path"].ToString().Replace("~/", "");

...
{
    public partial class DangKyXS2G : BasePage
    {
        private int width;
        private string lang;
        private int id;
            ...
            if (!Page.IsPostBack)
            {
                ...
                else
                {
                    ...
                    if (lang == "1")
                    {
                        ...
                        ltrSMS.Text = "Cung cấp kết quả xổ số <b> " + info.Rows[0]["company_name"].ToString().Replace("XS", "") + " </b> nhanh chóng, chính xác, thuận tiện. Kết quả sẽ được gửi ngay tới khách hàng sau khi mở thưởng. Đăng ký một lần nhận tin mãi mãi <br/> Miễn phí 7 lần trả KQ đầu tiên.<br/>Vui lòng truy cập bằng <b> 3G/GPRS </b> hoặc : <br/> SOẠN TIN :  <b> DK " + info.Rows[0]["company_comment"] + " </b> gửi <b> " + AppEnv.GetSetting("S2ShortCode") + "</b>";
                    }
                    else
                    {
                        ltrHuongdan.Text = "Ket qua xo so";
                        ltrSMS.Text = "Cung cap ket qua xo so <b> " + AppEnv.CheckLang(info.Rows[0]["company_name"].ToString().Replace("XS", "")) + " </b> nhanh chong, chinh xac, thuan tien. Ket qua se duoc gui ngay toi khach hang sau khi mo thuong. Dang ky mot lan nhan tin mai mai <br/> Mien phi 7 lan tra KQ dau tien.<br/>Vui long truy cap bang <b> 3G/GPRS </b> hoac : <br/> SOAN TIN : <b> DK " + info.Rows[0]["company_comment"] + " </b> gui <b> " + AppEnv.GetSetting("S2ShortCode") + "</b>";
                    }
                }
        ...
        protected void btnCo_Click(object sender, EventArgs e)
        {
            ...
            {
                Session["S2_Xs_Result_2G"] = "1";
                //string message = string.Format(AppEnv.GetSetting("S2MessageKQXS"), info.Rows[0]["company_comment"]);
                Response.Redirect(UrlProcess.GetS2RegisterResultXoSoUrl2G(lang, width.ToString(), id.ToString()));
            }
            else
            {
